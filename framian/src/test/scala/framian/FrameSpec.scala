...
class FrameSpec extends FramianSpec
  with PropertyChecks {
  ...
  "Frame" should {
    ...
    "have sane equality" in {
      ...
      f0 should !== (f1)
      f1 should !== (f0)
      f0.column[String](0).toFrame("abc") should === (f1.column[String](0).toFrame("abc"))
      f0.column[Int](1).toFrame("123") should !== (f1.column[Int](1).toFrame("123"))
    }

    "have sane hashCode" in {
      f0.hashCode should === (f0.hashCode)
      f0.hashCode should !== (f1.hashCode)
      f1.hashCode should !== (f0.hashCode)
      f0.column[String](0).toFrame("abc").hashCode should === (f1.column[String](0).toFrame("abc").hashCode)
      f0.column[Int](1).toFrame("123").hashCode should !== (f1.column[Int](1).toFrame("123").hashCode)
    }

    "sort columns" in {
      people.sortColumns should === (Frame.fromRows(
          32 :: "Manager"  :: "Bob" :: HNil,
    ...
    "sort rows" in {
      people.sortRows should === (Frame.fromRows(
          "Alice"   :: 24 :: "Employee" :: HNil,
          "Bob"     :: 32 :: "Manager"  :: HNil,
          "Charlie" :: 44 :: "Employee"  :: HNil)
        .withColIndex(Index.fromKeys("Name", "Age", "Level"))
    ...
    "use new row index" in {
      f0.withRowIndex(Index(0 -> 2, 1 -> 0, 2 -> 1)) should === (Frame.fromRows(
        "c" :: 3 :: HNil,
        "a" :: 1 :: HNil,
        "b" :: 2 :: HNil))
      f0.withRowIndex(Index(0 -> 0, 1 -> 0, 2 -> 0)) should === (Frame.fromRows(
        "a" :: 1 :: HNil,
        "a" :: 1 :: HNil,
        "a" :: 1 :: HNil))
      f0.withRowIndex(Index(0 -> 2)) should === (Frame.fromRows("c" :: 3 :: HNil))
      f0.withRowIndex(Index.empty[Int]) should === (Frame.empty[Int, Int].withColIndex(f0.colIndex))
    }

    "use new column index" in {
      f0.withColIndex(Index(0 -> 1, 1 -> 0)) should === (Frame.fromRows(
        1 :: "a" :: HNil,
        2 :: "b" :: HNil,
        3 :: "c" :: HNil))
      f0.withColIndex(Index(0 -> 0, 1 -> 0)) should === (Frame.fromRows(
        "a" :: "a" :: HNil,
        "b" :: "b" :: HNil,
        "c" :: "c" :: HNil))
      f0.withColIndex(Index(0 -> 1)) should === (Frame.fromRows(
        1 :: HNil,
        2 :: HNil,
        3 :: HNil))
      f0.withColIndex(Index.empty[Int]) should === (Frame.fromRows[HNil, Int](HNil, HNil, HNil))
    }

    "have trivial column/row representation for empty Frame" in {
      val frame = Frame.empty[String, String]
      frame.columnsAsSeries should === (Series.empty[String, UntypedColumn])
      frame.rowsAsSeries should === (Series.empty[String, UntypedColumn])
    }
  ...
  "Frame merges" should {
    ...
    "inner merge with frame of same row index" in {
      f3.merge(f4)(Merge.Inner) should === (
        Frame.fromRows(
          3 :: 3 :: HNil,
          2 :: 2 :: HNil,
    ...
    "outer merge with frame of same row index" in {
      f3.merge(f4)(Merge.Outer) should === (
        Frame.fromRows(
          3 :: 3 :: HNil,
          2 :: 2 :: HNil,
          1 :: 1 :: HNil
    ...
    "inner merge with an offset index with duplicates" in {
      f3.merge(f5)(Merge.Inner) should === (
        Frame.fromRows(
          2 :: 3 :: HNil,
          1 :: 2 :: HNil
    ...
    "outer merge with an offset index with duplicates" in {
      f3.merge(f5)(Merge.Outer) should === (
        Frame.fromRows(
          3  :: NA :: HNil,
          2  :: 3  :: HNil,
          1  :: 2  :: HNil,
    ...
    "inner merge with a smaller index with duplicates" in {
      f3.merge(f6)(Merge.Inner) should === (
        Frame.fromRows(
          2 :: 2 :: HNil,
          1 :: 1 :: HNil
        ).withRowIndex(Index(Array(2, 2))))
    ...
    "outer merge with a smaller index with duplicates" in {
      f3.merge(f6)(Merge.Outer) should === (
        Frame.fromRows(
          3 :: NA :: HNil,
          2 :: 2  :: HNil,
          1 :: 1  :: HNil
    ...
    "merge with a series" in {
      f3.merge(1, s1)(Merge.Inner) should === (
        Frame.fromRows(
          3 :: "s3" :: HNil,
          2 :: "s2" :: HNil,
          1 :: "s1" :: HNil
  ...
  "Frame joins" should {
    "inner join with empty frame" in {
      ...
      f0.join(e)(Join.Inner) should === (f0.withRowIndex(Index.empty[Int]))
      e.join(f0)(Join.Inner) should === (f0.withRowIndex(Index.empty[Int]))
      e.join(e)(Join.Inner) should === (e)
    }

    "inner join with series" in {
      f0.join(2, s0)(Join.Inner) should === (Frame.fromRows(
        "a" :: 1 :: "s3" :: HNil,
        "b" :: 2 :: "s2" :: HNil,
        "c" :: 3 :: "s1" :: HNil))
    }
    ...
    "inner join with self" in {
      f0.join(f0)(Join.Inner) should === (Frame.fromRows(
          "a" :: 1 :: "a" :: 1 :: HNil,
          "b" :: 2 :: "b" :: 2 :: HNil,
          "c" :: 3 :: "c" :: 3 :: HNil)
    ...
    "left join with empty frame" in {
      ...
      a.join(e)(Join.Left) should === (a)
      e.join(a)(Join.Left) should === (e.withColIndex(Index.fromKeys(0)))
    }
    ...
    "right join with empty frame" in {
      ...
      a.join(e)(Join.Right) should === (e.withColIndex(Index.fromKeys(0)))
      e.join(a)(Join.Right) should === (a)
    }
    ...
    "outer join with empty frame" in {
      ...
      a.join(e)(Join.Outer) should === (a)
      e.join(a)(Join.Outer) should === (a)
    }
  }

  "mapRowGroups" should {
    "not modify frame for identity" in {
      f0.mapRowGroups { (_, f) => f } should === (f0)
      f1.mapRowGroups { (_, f) => f } should === (f1)
    }
    ...
    "reduce groups" in {
      ...
      } should === (dups.reduceFrameByKey(reduce.Sum[Double]))
    }

    "replace groups with constant" in {
      val const = Frame.fromRows("repeat" :: HNil)
      dups.mapRowGroups { (_, f) => const } should === (Frame.fromRows(
        "repeat" :: HNil,
        "repeat" :: HNil,
        "repeat" :: HNil
  ...
  "Frame" should {
    "get row as HList" in {
      f0.get(Cols(0, 1).as[String :: Int :: HNil])(0) should === (Value("a" :: 1 :: HNil))
      f0.get(Cols(0, 1).as[String :: Int :: HNil])(1) should === (Value("b" :: 2 :: HNil))
      f0.get(Cols(0, 1).as[String :: Int :: HNil])(2) should === (Value("c" :: 3 :: HNil))
      f0.get(Cols(0, 1).as[String :: Int :: HNil])(3) should === (NA)
      f0.get(Cols(0).as[String :: HNil])(0) should === (Value("a" :: HNil))
      f0.get(Cols(1).as[Int :: HNil])(2) should === (Value(3 :: HNil))
    }

    "convert to series" in {
      f0.get(Cols(0).as[String]) should === (Series(0 -> "a", 1 -> "b", 2 -> "c"))
      f0.get(Cols(0).as[Int]) should === (Series(Index.fromKeys(0, 1, 2), Column[Int](NM, NM, NM)))
      f0.get(Cols(1).as[Int]) should === (Series(0 -> 1, 1 -> 2, 2 -> 3))
      f0.get(Cols(0, 1).as[String :: Int :: HNil]) should === (Series(
        0 -> ("a" :: 1 :: HNil),
        1 -> ("b" :: 2 :: HNil),
        2 -> ("c" :: 3 :: HNil)))
    }
    ...
    "map to series" in {
      f0.map(Cols(1).as[Int], 2)(_ + 1) should === (Frame.fromRows(
        "a" :: 1 :: 2 :: HNil,
        "b" :: 2 :: 3 :: HNil,
        "c" :: 3 :: 4 :: HNil))
      f0.map(Cols(0).as[String], 2)(_ => 42) should === (Frame.fromRows(
        "a" :: 1 :: 42 :: HNil,
        "b" :: 2 :: 42 :: HNil,
        "c" :: 3 :: 42 :: HNil))
      f0.map(Cols(1, 0).as[(Int, String)], 2) { case (x, y) =>
        y + x
      } should === (Frame.fromRows(
        "a" :: 1 :: "a1" :: HNil,
        "b" :: 2 :: "b2" :: HNil,
        "c" :: 3 :: "c3" :: HNil))
    }
    ...
    "map with index to series" in {
      f0.mapWithIndex(Cols(0).as[String], 2)(_ + _) should === (Frame.fromRows(
        "a" :: 1 :: "0a" :: HNil,
        "b" :: 2 :: "1b" :: HNil,
        "c" :: 3 :: "2c" :: HNil))
      f0.mapWithIndex(Cols(1).as[Int], 2)(_ + _) should === (Frame.fromRows(
        "a" :: 1 :: 1 :: HNil,
        "b" :: 2 :: 3 :: HNil,
        "c" :: 3 :: 5 :: HNil))
    }
    ...
    "filter whole frame" in {
      f0.filter(Cols(1).as[Int])(_ % 2 == 0) should === (
        Frame.fromRows("b" :: 2 :: HNil).withRowIndex(Index.fromKeys(1)))
    }

    "group by column values" in {
      f0.group(Cols(0).as[String]) should === (f0.withRowIndex(Index.fromKeys("a", "b", "c")))
      f0.group(Cols(1).as[Int].map(-_)) should === (f0.withRowIndex(Index(-3 -> 2, -2 -> 1, -1 -> 0)))
      f2.group(Cols(0).as[String]) should === (f2.withRowIndex(Index(("a",0), ("b",2), ("b",1))))
    }
  }

  "reduceFrameWithCol" should {
    "reduce with last" in {
      f0.reduceFrameWithCol[String, Int, (String, Int)](0)(reduce.Last) should === (
        Series(1 -> ("c", 3)))
    }
  }
  ...
  "map" should {
    "append column when to is new" in {
      f0.map(Cols(1).as[Int], to = 2)(_ + 2) should === (Frame.fromRows(
        "a" :: 1 :: 3 :: HNil,
        "b" :: 2 :: 4 :: HNil,
        "c" :: 3 :: 5 :: HNil))
    }
    ...
    "replace column when `to` exists" in {
      f0.map(Cols(1).as[Int], to = 1)(_ + 2) should === (Frame.fromRows(
        "a" :: 3 :: HNil,
        "b" :: 4 :: HNil,
        "c" :: 5 :: HNil))
    }
  ...
  "reduce" should {
    "reduce rows" in {
      f0.reduce(Cols(1).as[Double], 2)(reduce.Mean) should === (
        f0.merge(2, Series(0 -> 2D, 1 -> 2D, 2 -> 2D))(Merge.Outer))
    }

    "reduce cols" in {
      f0.transpose.reduce(Rows(1).as[Double], 2)(reduce.Mean) should === (
        f0.merge(2, Series(0 -> 2D, 1 -> 2D, 2 -> 2D))(Merge.Outer).transpose)
    }

    "replace column when `to` exists" in {
      f0.reduce(Cols(1).as[Int], to = 1)(reduce.Sum) should === (Frame.fromRows(
        "a" :: 6 :: HNil,
        "b" :: 6 :: HNil,
        "c" :: 6 :: HNil))

      f0.transpose.reduce(Rows(1).as[Int], to = 1)(reduce.Sum) should === (Frame.fromColumns(
        "a" :: 6 :: HNil,
        "b" :: 6 :: HNil,
        "c" :: 6 :: HNil))
    }
  ...
  "appendRows" should {
    "append rows to empty frame" in {
      Frame.empty[Int, Int].appendRows(f0) should === (f0)
      f0.appendRows(Frame.empty[Int, Int]) should === (f0)
    }

    "append 2 simple frames with same columns" in {
      f0.appendRows(f1) should === (Frame.fromRows(
        "a" :: 1 :: HNil,
        "b" :: 2 :: HNil,
        "c" :: 3 :: HNil,
        "a" :: 3 :: HNil,
    ...
    "append 2 simple frames with different columns" in {
      ...
      a.appendRows(b) should === (ColOrientedFrame(Index(Array(0, 1, 2, 0, 1, 2)),
    ...
    "append frame rows with same column oriented schema" in {
      ...
      forAll(Gen.zip(genFrame, genFrame)) { case (f0, f1) =>
        ...
        f0.appendRows(f1) should === (expected)
      }
    }
  }
}
