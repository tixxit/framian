...
class NumericColumnTyperSpec extends Specification {
  ...
  "NumericColumnTyper" should {
    ...
    "cast Int to anything" in {
      ...
      col.cast[Int].cells(0 to 4) must_== (values map (Value(_)))
      col.cast[Long].cells(0 to 4) must_== (values map (n => Value(n.toLong)))
      col.cast[Float].cells(0 to 4) must_== (values map (n => Value(n.toFloat)))
      col.cast[Double].cells(0 to 4) must_== (values map (n => Value(n.toDouble)))
      col.cast[BigInt].cells(0 to 4) must_== (values map (n => Value(BigInt(n))))
      col.cast[BigDecimal].cells(0 to 4) must_== (values map (n => Value(BigDecimal(n))))
      col.cast[Rational].cells(0 to 4) must_== (values map (n => Value(Rational(n))))
      col.cast[Number].cells(0 to 4) must_== (values map (n => Value(Number(n))))
    }

    "big Long cast to Int is not meaningful" >>
      checkCast[Long, Int]((MaxInt.toLong + 1) -> NM, MinLong -> NM, MaxLong -> NM)
    "small Long cast to Int is valid" >>
      checkCast[Long, Int](MaxInt.toLong -> Value(MaxInt), 0L -> Value(0), -100L -> Value(-100))
    checkBoundedInteger[Int](MinInt, MaxInt)

    "Int cast to Long is always valid" >>
      checkCast[Int, Long](MaxInt -> Value(MaxInt.toLong), 0 -> Value(0L), -100 -> Value(-100L))
    checkBoundedInteger[Long](MinLong, MaxLong)

    "Int cast to BigInt is always valid" >>
      checkCast[Int, BigInt](MaxInt -> Value(BigInt(MaxInt)), 0 -> Value(BigInt(0)), -100 -> Value(BigInt(-100)))

    "Long cast to BigInt is always valid" >>
